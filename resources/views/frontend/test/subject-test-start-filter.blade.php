                                ...
                                <div id="sidebar" class="clearfix">
                                    ...
                                    @php $studentQuestion = \App\StudentTestQuestion::where('id' , $studentTestQuestion->question_id)->first(); @endphp
                                        @if($studentQuestion->question_type == 1)
                                            ...
                                            @endphp
                                            <p class="mb-3 font-weight-bold text-dark"><span class="">{{ htmlspecialchars_decode(strip_tags($studentQuestion->body)) }} {{  </span></p>
                                            @foreach($getQuestions as $getQuestion)
                                                <div class="answer-option"><label class="ans-sec"><input type="radio" class="ans_button" data-buttonColor="{{ $a++ }}"  name="ans-{{ $studentTestQuestion->question_id }}"  value="{{ $getQuestion->id }}"> {{ htmlspecialchars(strip_tags($getQuestion->option_title)) }} <span class="checkmark"></span></label></div>
                                            @endforeach
                                        @endif                                 
                                    </div>
