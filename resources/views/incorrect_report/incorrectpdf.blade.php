...
<div class="card">
    <div class="card-header">
            <div class="float-left">
                <h3>Pdf Link Validation</h3>
            </div>
        </div>

            <div class="card-body">
            <form action="{{ route('incorrect_report.incorrectpdf') }}" method="POST" >
                @csrf
            <div class="container">                    
                <div class="row">                
                    ...
                    @if(count($courses)>0)    
                        ...
                        <table class="table table-bordered table-hover text-center">
                            ...
                            <tr>
                                ...
                                <th>@translate(Incorrect Pdf Link)</th>
                            ...
                            @foreach($courses as  $item)
                              ...
                              <td> {{$item->content_title}} </td> 
                                ...
                                <td class="text-danger"> @if ($item->file=='') <span> {{'Not Available'}} </span>  @endif
                                 <?php echo ltrim(($item->file), "uploads/class_contents/"); ?>
                                </td>

                                <td> <a href="#"  data-link="{{ $item->file }}" data-id="{{ $item->id }}"  class="text-secondary pdf_link" data-toggle="modal" 
                                                    data-target="#exampleModal"> <i class="la la-pencil"></i>Edit </a></td>
                                @endif
                                </tr>
                                @endforeach 
                            </tbody>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Update Pdf Link</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="{{ route('incorrect_report.update.pdf') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label>Pdf Link</label>
                <input type="file" class="form-control" name="pdf" accept="application/pdf" id="pdf" required>
...
$('#pdf').on( 'change', function() {
   ...
   if(ext=="pdf"){
    $('#btn').attr('disabled',false);
       ...
       swal("Oops!", "Only Pdf file is allowed", "warning");
   }
});
</script>
