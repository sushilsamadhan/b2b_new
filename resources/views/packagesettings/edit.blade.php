...
@section('content')
<?php // echo "<pre>";print_r($chkfree_subject);die; ?>
    <div class="card m-2">
        <div class="card-header">
            <div class="float-left">
                <h3>Update Package Setting</h3>
            </div>
        </div>
        <div class="card-body">
            <form action="{{ route('packagesettings.update',$packagesetting->id) }}" method="POST"  enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="id" value="{{$packagesetting->id}}">
                <div class="container">   
                    <div class="row">
                    <div class="form-group col-md-8 p-3">
                            <label class="" for="pkg_name">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="text"  class="form-control langr" placeholder="@translate(Enter Package Name)"  id="pkg_name" name="pkg_name" value="{{$packagesetting->pkg_name}}">
                                </div>
                        </div>
                    ...
                    <div class="form-group col-md-4 p-3">
                            ...
                            <div class="">
                                <select class="form-control langr selectpicker" id="package_type" name="package_type">
                                    ...
                                    <option value="board" {{ ($packagesetting->package_type == 'board')?'selected':'' }}>Board</option>
                                    <option value="competitive-courses" {{ ($packagesetting->package_type == 'competitive-courses')?'selected':'' }}>Competitive Courses</option>
                                </select>
                            </div>
                        </div>
                        {{-- Category --}}
                        ...
                        <label class="" for="val-category_id">
                            @translate(Board/Exam) <span class="text-danger">*</span></label>
                        <div class="">
                        <select class="form-control langr @error('category_id') is-invalid @enderror"
                                ...
                                @php $i=0; @endphp
                                    ...
                                    @if(count($category['child'])!=0)
                                            ...
                                            @else
                                            <option value="{{ $category->id }}" {{$packagesetting->category_id == $category->id ? 'selected':null}}>{{ $category->name }}</option>
                                            @endif
                                            @if(count($category['child'])!=0)
                                            @php $j=0; @endphp
                                            ...
                                            @foreach($category['child'] as $val)
                                                ...
                                                <option value="{{ $val->id }}" {{$packagesetting->category_id == $val->id ? 'selected':null}}>{{ $val->name }}</option>
                                                @php $j++; @endphp
                                    ...
                                    @else
                                        @foreach ($categories as $category)
                                            ...
                                            @else
                                                <option value="{{ $category->id }}" {{ ($packagesetting->category_id == $category->id)?'selected':'' }}>{{ $category->name }}</option>
                                            @endif
                                            @if(count($category['child'])!=0)
                                            @php $j=0; @endphp
                                                ...
                                                @foreach($category['child'] as $val)
                                                    ...
                                                    <option value="{{$category->id}}-{{ $val->id }}" {{ ($packagesetting->sub_category_id == $val->id)?'selected':''}}>{{ $val->name }} </option>
                                                    @php $j++; @endphp
                   ...
                   @endif
                            ...
                            <div class="">
                                <select class="form-control langr @error('sub_category_id') is-invalid @enderror"
                                        ...
                                        @foreach($subCatdetail as $subCat)
                                             <option value="{{$subCat->id}}" {{ ($packagesetting->sub_category_id == $subCat->id)?'selected':'' }}>{{$subCat->name}}</option>
                                        @endforeach
                                </select>
                            </div>
                    </div>
                        ...
                        <div class="form-group col-md-4 p-3" id="board_view" >
                            ...
                            <div class="">
                                <select class="form-control langr @error('course_id') is-invalid @enderror"
                                       ...
                                       <option value="0" {{($packagesetting->is_all_subject=='1')?'selected':''}}>All subject</option>
                                        @foreach($courses_detail as $courseVal)
                                             <option value="{{$courseVal->id}}" {{ ($packagesetting->course_id == $courseVal->id &&  $packagesetting->is_all_subject !='1')?'selected':'' }}>{{$courseVal->title}}</option>
                                        @endforeach
                                </select>
                            </div>
                    </div>
                    ...
                    <div class="form-group col-md-4 p-3">
                            <label class="" for="quarterly_course_coverage">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" placeholder="@translate(Enter Quarterly Course Coverage(%))"  id="quarterly_course_coverage" name="quarterly_course_coverage" value="{{$packagesetting->quarterly_course_coverage}}">
                                </div>
                        </div>
                        <div class="form-group col-md-4 p-3">
                            <label class="" for="halfyrly_course_coverage">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" placeholder="@translate(Enter Half Yearly Course Coverage(%))"  id="halfyrly_course_coverage" name="halfyrly_course_coverage" value="{{$packagesetting->halfyrly_course_coverage}}">
                                </div>
                        </div>
                        ...
                        <div class="form-group col-md-4 p-3">
                            <label class="" for="annually_course_coverage">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" placeholder="@translate(Enter Annually Course Coverage(%))"  id="annually_course_coverage" name="annually_course_coverage" value="{{$packagesetting->annually_course_coverage}}">
                                </div>
                        </div>
                        ...
                        <div class="form-group col-md-4 p-3">
                            <label class="" for="quarterly_coverage_price">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" placeholder="@translate(Enter Quarterly Coverage Price)"  id="quarterly_coverage_price" name="quarterly_coverage_price" value="{{$packagesetting->quarterly_coverage_price}}">
                                </div>
                        </div>
                        <div class="form-group col-md-4 p-3">
                            <label class="" for="halfyrly_coverage_price">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" placeholder="@translate(Enter Half Yearly Coverage Price)"  id="halfyrly_coverage_price" name="halfyrly_coverage_price" value="{{$packagesetting->halfyrly_coverage_price}}">
                                </div>
                        </div>
                        <div class="form-group col-md-4 p-3">
                            <label class="" for="annually_coverage_price">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" placeholder="@translate(Enter Annullay Coverage Price)"  id="annually_coverage_price" name="annually_coverage_price" value="{{$packagesetting->annually_coverage_price}}">
                                </div>
                        </div>
                        <div class="form-group col-md-4 p-3">
                            <label class="" for="default_discount">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" placeholder="@translate(Enter Default Discount)"  id="default_discount" name="default_discount" value="{{$packagesetting->default_discount}}">
                                </div>
                        </div>
                        ...
                        <div class="form-group col-md-4 p-3">
                            <label class="" for="annually_coverage_price">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" placeholder="@translate(Enter Member Discount)"  id="member_discount" name="member_discount" value="{{$packagesetting->member_discount}}">
                                </div>
                        </div>
                        ...
                        <div class="form-group col-md-4 p-3" id="chapter">
                            <label class="" for="annually_coverage_price">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" title="@translate(Enter Number Of Chapter Test)" placeholder="@translate(Enter Number Of Chapter Test)"  id="no_of_test" name="no_of_test" value="{{$packagesetting->no_of_test}}">
                                </div>
                        </div>
                        ...
                        <div class="form-group col-md-4 p-3" id="unit">
                            <label class="" for="no_of_practice_test">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" title="@translate(Enter Number Of Unit Test for k-12)" placeholder="@translate(Enter Number Of Unit Test for k-12)"  id="no_of_practice_test" name="no_of_practice_test" value="{{$packagesetting->no_of_practice_test}}">
                                </div>
                        </div>
                        ...
                        <div class="form-group col-md-4 p-3">
                            <label class="" for="no_of_sectional_test">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" title="@translate(Enter Number Of Subject Test for competitive and k-12)" placeholder="@translate(Enter Number Of Subject Test for competitive and k-12)"  id="no_of_sectional_test" name="no_of_sectional_test" value="{{$packagesetting->no_of_sectional_test}}">
                                </div>
                        </div>
                        ...
                        <div class="form-group col-md-4 p-3">
                            <label class="" for="no_of_test_questions">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <input type="number"  class="form-control langr" placeholder="@translate(Enter Number Of Test question for K-12 segment else 0)"  id="no_of_test_questions" name="no_of_test_questions" value="{{ ($packagesetting->no_of_test_questions == '0')?'0':$packagesetting->no_of_test_questions }}">
                                </div>
                        </div>
                        <div class="form-group col-md-4 p-3">
                            ...
                            <div class="form-group">
                                <label class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="status"
                                        value="0" {{ ($packagesetting->status == '0')?'checked':'' }}>
                                    <span class="form-check-label">In-Active</span>
                                </label>
                                <label class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="status"
                                        value="1" {{ ($packagesetting->status == '1')?'checked':'' }}>
                        ...
                        {{-- Course Thumbnail --}}
                            <div class="form-group col-md-4 p-3">
                                <label class="col-lg-5 col-form-label" for="val-img">
                                    ...
                                    <div class="col-lg-9">
                                        <img src="{{ filePath($packagesetting->pkg_image) }}" width="200" height="auto" alt="photo">
                                        <br>

                                        <input type="hidden" required value="{{$packagesetting->pkg_image}}" class="form-control course_image @error('pkg_image') is-invalid @enderror" id="val-img" name="pkg_image">
                                        <img class="course_thumb_preview rounded shadow-sm d-none" src="" alt="#Course thumbnail" width="200" height="auto">
                                        @error('pkg_image') <span class="invalid-feedback" role="alert"> <strong>{{ $message }}</strong> </span> @enderror
                                    ...
                                    <select class="form-control" id="addon_service_id" name="addon_service_id[]" size="5" multiple="multiple" onChange="addOnServices(this.value)">
                                        <option value="err"> @translate(Use Ctrl To Choose Multiple Service)</option>
                                            @foreach($service as $serviceVal)
                                               ...
                                               @php 
                                               $chkVal = getSelected($packagesetting->id,$serviceVal->id); @endphp

                                                        @if($chkVal)
                                                            <option value="{{$serviceVal->id.'__'.$serviceVal->price}}" selected>{{$serviceVal->service_name}} - &#x20B9;{{round($serviceVal->price,2)}}</option>
                                                           @else
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-4 p-3" id="free_view_subject">
                                ...
                                <div class=""> 
                                    ...
                                    <select class="form-control langr @error('free_subject') is-invalid @enderror"
                                            ...
                                            @foreach($free_courses_detail as $freeVal)
                                                <option value="{{$freeVal->id}}" 
                                                    @php 
                                                       if(in_array($freeVal->id, $chkfree_subject))
                                                            { 
                        ...
                        <div class="form-group col-md-12 p-3">
                            <label class="" for="pkg_desc">
                                @translate(Select Demo Course) <span class="text-danger"></span></label>
                            <select class="form-control langr" name="demo_course_id" id="demo_course_id">
                            <option value="">@translate(Please Select Demo Course)</option>
                             @if(isset($demoCourse) && count($demoCourse)>0)
                                @foreach($demoCourse as $value)
                                    <option value="{{$value->id}}" {{($packagesetting->demo_course_id==$value->id)? 'selected':''}}>{{$value->title}}</option>
                                @endforeach
                             @else 
                        ...
                        <div class="form-group col-md-12 p-3">
                            <label class="" for="short_desc">
                                ...
                                <div class="">
                                    <textarea class="form-control langr" placeholder="@translate(Enter Package Short Description)"  id="short_desc" name="short_desc" >{{$packagesetting->short_desc}}</textarea>
                                </div>
                        </div> 
                        ...
                        <div class="form-group col-md-12 p-3">
                            <label class="" for="pkg_desc">
                                ...
                                <div class="bootstrap-tagsinput">
                                    <textarea class="form-control langr" placeholder="@translate(Enter Package Description)"  id="pkg_desc" name="pkg_desc">{{$packagesetting->pkg_desc}}</textarea>
                                </div>
                        </div>
...
<script type='text/javascript'>
        ...
        $(document).ready(function(){
            ...
            var contentType = $('#package_type').val();
            if(contentType=='board')
            {  
                categoryIid = $('#val-category_id').val(); 

                $("#sub_category_id_display").css("display", "block");
                $("#board_view").css("display","block");
                $.ajax({
                type:"get",
                url:"{{url('dashboard/packagesettings/categoriesById')}}/"+categoryIid,
