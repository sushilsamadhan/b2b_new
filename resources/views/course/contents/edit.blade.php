...
<form id="content_form" action="{{ route('classes.contents.contentUpdate') }}" method="post" enctype="multipart/form-data">
    @csrf
    {{-- Title --}}
    ...
    <div class="form-group row">
        ...
        <div class="col-lg-9">
            <select class="form-control @error('content_type') is-invalid @enderror" required id="val-Content"
                ...
                <option value="Video" {{($content->content_type=='Video')?'selected':''}} id="youtube">
                    @translate(Video)
                </option>
                <option value="Document" id="vimeo" {{($content->content_type=='vimeo')?'selected':''}}>
                    @translate(Document)
                </option>
                @if(quizActive())
                    <option value="Quiz" {{($content->content_type=='Quiz')?'selected':''}}>
                        @translate(Quiz)
                    </option>
                @endif
            </select>
            ...
            <div class="form-group row">
                ...
                <div class="col-lg-9">
                    <select class="form-control lang @error('provider') is-invalid @enderror" id="val-provider"
                        ...
                        <option value="Youtube" {{($content->quiz_id=='Youtube')?'selected':''}}>
                            @translate(Youtube)
                        </option>
                        <option value="HTML5" {{($content->quiz_id=='HTML5')?'selected':''}}>
                            @translate(HTML5)
                        </option>
                        <option value="Vimeo" {{($content->quiz_id=='Vimeo')?'selected':''}}>
                            @translate(Vimeo)
                        </option>
                        <option value="File" id="file" {{($content->quiz_id=='File')?'selected':''}}>
                            @translate(File)
                        </option>
                        <option value="Live" {{($content->quiz_id=='Live')?'selected':''}}>
                            @translate(Live)
                        </option>
                    </select>
                    ...
                    @if (zoomActive())
                        ...
                        <select class="form-control lang @error('meeting_id') is-invalid @enderror" id="val-meeting"
                            ...
                            @foreach (App\Meeting::where('user_id', Auth::user()->id)->latest()->get() as $course)
                                <option value="{{ $course->id }}"  {{($content->meeting_id==$course->id)?'selected':''}}>
    ...
    <div class="">
        <div class="form-group row is-invalid">
            ...
            <div class="col-lg-9">
                ...
                <img class="w-100 source_code_preview rounded shadow-sm d-none" src="{{$content->source_code}}" alt="#Source code">

                <input type="hidden" name="source_code_url" class="source_code_url" value="{{$content->source_code}}">
    ...
    {{-- Demo Type --}}
        ...
        <label class="col-lg-3 col-form-label">Choose Demo Type</label>
            <label class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="demo_type"
            ...
            @if(isset($content->demo_type) && $content->demo_type!='')
                <a href="#" id="remove-demo-url">Remove Demo Url</a>
            @endif
        </div> <!-- form-group end.// -->
        ...
        <div class="col-lg-9">
                    <div class="input-group mb-3">
                        ...
                        <input type="text" value="{{ $content->demo_url }}" name="demo_url" id="demo_url" class="form-control @error('demo_url') is-invalid @enderror">
    ...
    <div class="form-group row">
        ...
        <div class="col-lg-9">
            <input type="number" min="0" class="form-control @error('demo_duration') is-invalid @enderror" name="demo_duration" id="demo_duration"
...
<script>
    ...
    $(document).ready(function() {
        setTimeout(function(){ $('#val-Content').trigger("change"); $('#val-provider').val('{{$content->provider}}')}, 1000);
    });
    $(document).on('click','#remove-demo-url',function(){
        $('[name=demo_type]').prop('checked',false);
        $('[name=demo_duration]').val('');
        $('[name=demo_url]').val('');
    });
</script>
