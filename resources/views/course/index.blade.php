    ...
    <div class="card m-b-30">
        ...
        <div class="card-body">
                    ...
                    @forelse ($courses as $course)
                        <tr>
                            ...
                            @php
                                ...
                                $categoryName = '';
                                if($parent_id!='0'){
                                    $categoryDetail = \App\Model\Category::where('id',$parent_id)->first();
                                    if($categoryDetail){
                                        $parentCategoryName = $categoryDetail->name;
                                        
                                        if($categoryDetail->is_compitative=='1'){
                                            $categoryName = 'Competitive';
                                        }elseif($categoryDetail->is_free_study=='1'){
                                            $categoryName = 'Free Study Material';
                                        }elseif($categoryDetail->is_current_affairs=='1'){
                                            $categoryName = 'Current Affairs';
                                        }elseif($categoryDetail->is_project_works=='1'){
                            ...
                            {{$course->relationBetweenCategory->name }}</span></td>
                            <td>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right st-drop"
                                        ...
                                        @if(Auth::user()->id != '1300')
                                            <a class="dropdown-item font-13"
                                            href="{{ route('course.lesson_details',[$course->slug])}}">
            ...
            @if(Auth::user()->user_type == 'Admin' || Auth::user()->id == '5')
                @php
                    $string = \App\Http\Controllers\Course\CourseController::getCourseDemoIsExist($course->id);
                    $result = explode('|',$string);
                    $demoCourseId =  $result[0];
                    $demoCourseSlug =  $result[1];
                @endphp           
                                            @if($course->is_demo==0 && $demoCourseId==0)
                                                @if(Auth::user()->id != '1300')
                                                <a class="dropdown-item font-13"
                                                href="{{ route('course.demo_course',[$course->id])}}">
                                                    @translate(Create Demo Course)
                                                </a>
                                                @endif
                                            @else
                                                <a class="dropdown-item font-13"
                                                href="{{ route('course.show',[$demoCourseId,$demoCourseSlug])}}">
                                                    @translate(View Demo Course)
                                                </a>    
                                            @endif
            @endif    
                                        @if($s >= 0)
