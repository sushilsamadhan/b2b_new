<div class="card-body">
    <form action="{{route('free-study-material.update')}}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-group">
            ...
            <input value="{{ $category->icon }}" name="icon" class="icon" type="hidden">
                ...
                @if (MediaActive())
                    {{-- media --}}
                    <a href="javascript:void()" onclick="openNav('{{ route('media.slide') }}', 'freestudymaterial')" class="btn btn-primary media-btn mt-2 p-2">Upload From Media <i class="fa fa-cloud-upload ml-2" aria-hidden="true"></i> </a>
                @endif
