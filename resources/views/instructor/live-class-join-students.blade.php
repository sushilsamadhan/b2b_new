                ...
                <div class="col">
                    <form method="get" action="">
                        <div class="input-group">
                            <input type="text" name="search" class="form-control col-12" placeholder="@translate(Search by title)" value="{{Request::get('search')}}">
        ...
        <table class="table table-hover table-bordered">
            <thead>
                <tr class="text-center">
                    ...
                    <th>@translate(Join Time)</th>
                    <th>@translate(Left Time)</th>
                    <th>
                        @translate(Title)</th>
                    <th>
            ...
            <tbody class="text-center">
                ...
                <tr>
                    ...
                    <td>{{ $item->sname }}</td>
                    <td>
                        @if($item->time_of_join == '')
                        {{ 'NA' }}
                        @endif
                        {{ $item->time_of_join }}</td>
                    <td>
                        @if($item->time_of_left == '')
                        {{ 'NA' }}
                        @endif
                        {{ $item->time_of_left }}</td>
                   ...
                   {{-- @if($item->instructorDetail)
                    ...
                    <td>@php $subject = \App\Model\Course::where('id' , $item->instructor_subject_id)->first()@endphp
                    @if($subject)
                        {{ $subject['title'] }}
                    @else
                    @php $subject = \App\QuestionTag::where('id' , $item->instructor_subject_id)->first()@endphp
                    {{ $subject['tag_name'] }}
                    @endif
                    </td>
                    <td>{{$item->date}}</td>
