                ...
                <div class="col-lg-4">
                    ...
                    <div class="package-thumbnail">
                            ...
                            @if($s_course->lms_refference_id == 0 && $s_course->ole_refference_id > 0)
                                <img class="lazy" src="https://olexpert.org.in/public/{{$s_course->image}}"
                                    alt="https://olexpert.org.in/public/uploads/{{$s_course->image}}">
                            @endif
                            @if($s_course->lms_refference_id > 0 && $s_course->ole_refference_id > 0)
                                <img class="lazy" src="https://courses.iid.org.in/public/{{$s_course->image}}"
                                    alt="https://courses.iid.org.in/public/uploads/{{$s_course->image}}">
                            @endif
                            @if($s_course->ole_refference_id == 0 && $s_course->lms_refference_id == 0)
                                <img class="lazy" src="{{url('public')}}/{{$s_course->image}}"
                                    alt="{{url('public')}}/{{$s_course->image}}">
                            @endif    
                            ...
                            <div class="course-widget-buttons mt-2">
                                ...
                                @auth()
                                    @if(\Illuminate\Support\Facades\Auth::user()->user_type == 'Student')
                                        @if($s_course->content_type=='free-study-material')
                                            <a href="#!"
                                            class="template-button  btn btn-block btn-warning text-dark addToCart-{{$s_course->ole_refference_id}}"
                                            onclick="addToCart({{$s_course->ole_refference_id}},'{{route('add.to.cart')}}')">@translate(Checkout For Free)</a>
                                        @else
                                        <a href="#!"
                                            class="template-button  btn btn-block btn-warning text-dark addToCart-{{$s_course->ole_refference_id}}"
                                            onclick="addToCart({{$s_course->ole_refference_id}},'{{route('add.to.cart')}}')">@translate(Add to cart)</a>
                                        @endif
                                    @else
                                        @if($s_course->content_type=='free-study-material')
