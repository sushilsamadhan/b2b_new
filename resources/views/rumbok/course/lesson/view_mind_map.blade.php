...
@foreach($getMinMaps as $valImg)
    ...
    <li><a href="{{route('lesson.view_mind_map',[$valImg->class_content_id,$otherMindMapString])}}" class="{{$cId==$valImg->class_content_id?'active':''}}">{{$classContent->title}}</a></li>
@endforeach
  @foreach($otherMinMaps as $content)
    <?php $classContent = \App\Model\ClassContent::find($content->class_content_id);?>
    <li><a href="{{route('lesson.view_mind_map',[$content->class_content_id,$otherMindMapString])}}" class="{{$cId==$content->class_content_id?'active':''}}">{{$classContent->title}}</a></li>
  @endforeach
</ul>
    </div>
    ...
    <!-- Course Category Section Starts -->
        ...
        <a href="{{$lesson_page_url}}" style="background:#000;color:#fff;font-size: 13px;font-weight:bold;text-align:center;display:block;padding: 0px 10px;text-decoration:none;position: fixed;z-index: 9999;top: 2px;left: 10px;border-radius: 3px;">Back</a>
