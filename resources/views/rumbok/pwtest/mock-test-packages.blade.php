                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3">
                            ...
                            @if($type == 'mockTest')
                                <div class="card__content">
                                    ...
                                    <h3 class="card__header">{{$mockTests->name}} -Mock Test</h3>
                                    <div class="card__info">
                                              @if($getTestReports->count() != 0)
                                                            @if($getTestReports->count() < $packageDetail->no_of_sectional_test )
                                                        <p> <span class="badge bg-warning text-dark"> {{ $getTestReports->count() }}/{{ $packageDetail->no_of_sectional_test }}</span>    </p>  
                                                            <p><a href="{{ route('pw-mock-test-detail',[$packageDetail->id , $mockTests->id]) }}" class="btn btn-success">Re-attempt</a> </p>
                                                            @else
                                                            <p><span class="badge bg-warning text-dark"> {{ $getTestReports->count() }}/2</span></p>
                                                            <p class="btn btn-success">Completed</p> 
                                                            @endif
                                                        @else
                                                        <p><span class="badge bg-warning text-dark"> 0/{{ $packageDetail->no_of_sectional_test }}</span></p> 
                                                                                    
                                                        <p><a href="{{ route('pw-mock-test-detail',[$packageDetail->id , $mockTests->id]) }}" class="btn btn-primary">Attempt</a></p>
                                                        @endif
                                    </div>
                                </div>
                        ...
                        @if($path)
                         <i class="fas fa-arrow-left"></i> 
                            <input type="button" value="Back" class="btn btn-default" data-dismiss="modal" id="btnHome" onClick="document.location.href='{{ url('')}}/project_work_lesson_details/{{$path}}/{{$path1}}'" />
                        @else
                        ...
                        <div class="col-md-9">
                            <div class="row">
                              @foreach($getTestReports as $getTestReport)
                                ...
                                <div class="col-lg-6">
                                  <a href="{{ route('pw-mock-analytical-report' , [$packageDetail->id , $getTestReport->id]) }}" class="data-card">

                                  @php $getCreated =  getCreatedAtAttribute($getTestReport->created_at) ;@endphp
                                      
                                  @php
                                  $studentTestQuestions = \App\MockTestEnrollmentAnswer::where(['mock_test_enrollment_id' => $getTestReport->id , 'package_id'=> $packageDetail->id , 'user_id' => $getTestReport->user_id])->get();

                                   $getTotalMarks = getTotalMarks($getTestReport->id ,$packageDetail->id);  
                                   
                                   $newPercentage = ($getTotalMarks / $studentTestQuestions->count()) * 100
                                   @endphp
                                  <h3>{{ $getCreated }} <span class="badge badge-primary float-right"> {{ ($newPercentage>=60)?'Passed':'Fail'}}</span></h3>          
                                  
                                  <h4>No. of questions - {{ $studentTestQuestions->count() }}</h4>
