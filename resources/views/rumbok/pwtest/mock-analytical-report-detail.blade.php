        ...
        <div class="row">
            <div class="col-lg-6">
                <div class="title-page">
                    @php $countAttempt = \App\MockTestEnrollment::where(['user_id' => $mockTestEnrollment->user_id ,'package_id' => $packageDetail->id, 'mock_test_id' => $mockTestEnrollment->mock_test_id] )->where('id' ,'<=', $mockTestEnrollment->id)->get();
                        @endphp
                        <h1>{{ $packageDetail->pkg_name }} Mock Test - Attempt {{ $countAttempt->count() }}</h1>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="bread-crumb-part">
                    <a href="{{ route('pw-mock-report-detail' ,[$packageDetail->id, $mockTestEnrollment->id]) }}" type="botton" class="btn btn-danger">View Solution</a>
                </div>
            </div>
            <div class="col-lg-2">
                <div class="clearfix">
                    ...
                    <input type="button" value="Back" class="btn btn-success" data-dismiss="modal" id="btnHome" onClick="document.location.href='{{ url('')}}/project_work_lesson_details/{{$path}}/{{$path1}}'" />
                    @else
                    <a href="javascript:history.back()" class="btn btn-success"><span class="link-text">
                            Back
...
$newPercentage = ($getTotalMarks / $studentTestQuestions->count()) * 100 ; @endphp
<div class="col-md-3">
                <div class="analytic-card-1 {{ ($newPercentage>=60)?'light-red':'dark-red'}}">
                    <div class="d-flex align-items-center">
                        <div class="heading-and-count">
                            <div class="analytic-card-1-heading">Status</div>
                            <div class="analytic-card-1-count">{{ ($newPercentage>=60)?'Passed':'Fail'}}</div>
                        </div>
                ...
                <div class="analytic-card-1 blue">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="heading-and-count">
                            ...
                            <div class="analytic-card-1-count">{{ $getTotalMarks }}/{{ $studentTestQuestions->count()}}</div>
                        </div>
                        <div class="icon-analytic-card-1 ml-auto">
                            <i class="ti-flag"></i>
                        </div>
                ...
                <div class="analytic-card-1 sea-blue">
                    <div class="d-flex align-items-center">
                        <div class="heading-and-count">
                            ...
                            $Totelatt = ($studentTestQuestionsAttend->count() - $totalIncorrect);
                            $accuracy = 0;
                            if($Totelatt != 0){
                            $accuracy = $Totelatt /$studentTestQuestionsAttend->count() *100;
                            }
                            @endphp
                            <div class="analytic-card-1-heading">Accuracy</div>
                            <div class="analytic-card-1-count">{{ round($accuracy , 2) }} %</div>
                        </div>
                        <div class="icon-analytic-card-1 ml-auto">
                ...
                <div class="analytic-card-1 light-red">
                    <div class="d-flex align-items-center">
                        <div class="heading-and-count">
                            ...
                            <div class="analytic-card-1-count">{{ round($newPercentage, 2 )}}%</div>
                        </div>
                        <div class="icon-analytic-card-1 ml-auto">
                        ...
                        <div class="Rtable Rtable--5cols Rtable--collapse">
                            ...
                            <div class="Rtable-row">
                                <div class="Rtable-cell date-cell">
                                    ...
                                    @php $getUnit = \App\MockTestSection::where('id', $arrayValue)->first(); @endphp
                                    <div class="Rtable-cell--content date-content"><span class="webinar-date">Unit{{ $i++ }} - {{ $getUnit->section_name }}</div>
                                </div>
                                @php $getQuestionsssss =0;
                                $getquestion = 0;
                                //print_r($totalAttend); die;

                                foreach($totalAttend as $total ){
                                if($total['section_id'] == $arrayValue && $total['answer_id'] != ''){
        ...
        <div class="row mt-3">
            <div class="col-md-12">
                ...
                $i = $j = $k = 1;
                foreach ($arrayValues as $key => $arrayValue) {
                    $totalQuestion  =  array_count_values(array_column($totalAttend, 'section_id'))[$arrayValue];

                    $getUnit = \App\MockTestSection::where('id', $arrayValue)->first();
                    $dataPoints1[] =  array("label" => 'Unit' . $i++ . '-' . $getUnit->section_name, "y" => $totalQuestion);
                    $getQuestionsssss = 0;
                    $getquestion = 0;
                    foreach ($totalAttend as $total) {
                        if ($total['section_id'] ==  $arrayValue && $total['answer_id'] != '') {
                    ...
                    $dataPoints2[] =
                        array("label" => 'Unit' . $j++ . '-' . $getUnit->section_name, "y" => $getquestion);

                    $dataPoints3[] =
                        array("label" => 'Unit' . $k++ . '-' . $getUnit->section_name, "y" => $getQuestionsssss);
                }
