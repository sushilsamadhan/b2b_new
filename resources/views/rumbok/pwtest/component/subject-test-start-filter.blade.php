<div class="col-lg-9">
    ...
    <div class="row">
        <div class="col-lg-9">
            ...
            <p class="font-weight-bold mr-3">Unit Test- {{ $unitName->title }} </p>
            @elseif(Request::segment(1) == 'chapter-test-start')
            <p class="font-weight-bold mr-3">Chapter Test- {{ $unitName->title }}</p>
            @else
            <p class="font-weight-bold mr-3">Subject Test</p>
            @endif
            <span id="showMessage" class="text-center" style="color:red;"></span>
        </div>
        <div class="col-lg-3">
            <div class="row">
                <div class="col-md-4">
    ...
    <div class="content-holder">
        ...
        <div class="clearfix">
            <div role="tabpanel" class="clearfix" id="work">
                @php $i = 1;
                $j = 1;
                $k = 1;
                $p =1;
                ...
                <div class="clearfix content" id="content-{{$j++ }}" data-count="{{ $studentTestQuestions->count() }}" data-question="{{ $studentTestQuestion->question_id }}" data-section="{{ $packageDetail->id }}" data-id="{{ $k++ }}" style={{ ($p++ == 1) ? "display:block!important" : 'display:none!important'}}>
                    <div class="row">
                        <div class="col-md-5">
                            Question {{ $i++}}:
                        </div>
                        ...
                        <div class="col-md-4 text-right  col-7">
                            <button class="button">Marks:<span style="color:green;">+1 </span> <span style="color:red;">-0 </span></button>&nbsp; &nbsp;
                            <a href="#" class="GetReport" data-package_id="{{ $packageDetail->id }}" data-question_id="{{ $studentTestQuestion->question_id }}" data-toggle="modal" data-target="#myModal{{ $studentTestQuestion->question_id }}">Report</a>
                        </div>
                    </div>
                    <hr width="100%">
                    <div id="sidebar" class="clearfix position-relative">
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                @if( $studentQuestion != '' && $studentQuestion->question_type == 1)
                                @php
                                $getQuestions = \App\Option::where('question_id',$studentTestQuestion->question_id)->get();
                                @endphp
                                <p class="mb-3 font-weight-bold text-dark"><span class=""> <?php echo html_entity_decode($studentQuestion->body) ?> </span></p>
                                <div class="answer-option mt-3">
                                    @foreach($getQuestions as $getQuestion)
                                    <label class="ans-sec"><input type="radio" class="ans_button" id="form_id" data-buttonColor="{{ $a++ }}" name="ans-{{ $studentTestQuestion->question_id }}" value="{{ $getQuestion->id }}"> <?php echo  html_entity_decode($getQuestion->option_title) ?> <span class="checkmark"></span></label>
                                    @endforeach
                                </div>
                                @endif
                            </div>
                            @if($studentQuestion != '' && $studentQuestion->question_type == 3)
                        ...
                        <div class="modal-content">
                            ...
                            <form method="POST">
                                ...
                                <div class="modal-body">
                                    ...
                                    @if(Request::segment(1) == 'unit-test-start')
                                    <input type="hidden" id="test_type" name="type" value="unit">
                                    @elseif(Request::segment(1) == 'chapter-test-start')
                                    <input type="hidden" id="test_type" name="type" value="chapter">
                                    @else
                                    <input type="hidden" id="test_type" name="type" value="subject">
                                    @endif

                                    <div class="col-md-12">
                                        <select class="form-control" name="type_error" id="type_error">
                                    ...
                                    <div class="col-md-12 mt-4">
                                        <textarea class="form-control" name="error" id="error" row="10"></textarea>
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" id="questionReport" class="btn btn-primary">Submit
                                        Report</button>
        ...
        <div class="clearfix py-3 mt-3 border-top border-bottom custom-btn-div">
            <div class="row">
