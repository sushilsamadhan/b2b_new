...
<div id="containerbar">
    ...
    @if(Auth::user()->user_type == "Instructor" && Auth::user()->is_external == '3')
        @include('layouts.urlpermission_leftsidebar')
    @else
...
<!-- End Leftbar -->
    ...
    <!-- End Topbar -->
        ...
        @elseif(Auth::user()->user_type == "Instructor" && Auth::user()->is_external == '3')
            @php
                $site_url2 = \URL::current();
                $current_url2 = explode('dashboard',$site_url2);
            
                $result = getUrlAccessPermission(Auth::user()->id);

                $live_url = permission_denied();
                $prevoius_url = URL::previous();
                if($result){ 

                    $dynamic_url = explode(',',trim($result->site_url));

                    if(!in_array($live_url,$dynamic_url)){

                        echo '<script>
                            alert("You have no permission to access this url.");
                            window.location.href = "'.$prevoius_url.'";
                        </script>';
                    } 
                }else{
                    echo '<script>
                            window.location.href = "'.$current_url2[0].'dashboard/redirect";
                        </script>';
                }
            @endphp
        @endif
