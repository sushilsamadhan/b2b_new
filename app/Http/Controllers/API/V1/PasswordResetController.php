...
class PasswordResetController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $user = User::where('email', $request->phone_no)->where('user_type','Student')->first();
    ...
    public function reset(Request $request)
    {
        ...
        $customMessages = [
            'phone_no.required' => 'The Phone no  is required or unique.',
        ...
        $passwordReset = ApiPasswordReset::where('token', $request->token)->where('email', $request->phone_no)->first();
        if (!$passwordReset)
            return response()->json([
                'message' => 'This password reset token is invalid.'
            ], 404);
